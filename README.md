...
| 🕐 | **Coding hours heatmap** | 7×24 grid showing when a user typically pushes code, by hour and day of week |
...
User enters username
  ...
  Promise.all([
    ...
    fetchUserEvents()     → Worker → KV /users/{u}/events (2 pages, up to 200 events)
  ])
        │
        ▼
...
| Route | TTL | Why |
|---|---|---|
| `/rate_limit` | 1 min | Live quota - needs to stay fresh |
...
| `/users/:u/events` | 15 min | Recent push events for the coding hours heatmap |
